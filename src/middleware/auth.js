...
const authentication = async function (req, res, next) {
    try {
        ...
        if (!token) return res.status(400).send({ status: false, msg: "provide token in request hadder in form of Bearear token " })
        ...
        token = token[1].trim()
        let validateToken = jwt.verify(token, "jyoti Pratap")
